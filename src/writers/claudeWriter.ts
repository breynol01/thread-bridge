 ...
 */
function resolveModel(session: IRSession): string {
...
export function writeClaudeSession(
  session: IRSession,
  options?: { dryRun?: boolean; name?: string; cwd?: string },
): ClaudeWriteResult {
  ...
  if (!slug) {
    ...
    const text =
      ...
      firstUserMsg?.content
        ...
        .join(" ");
    if (text) slug = generateSlug(text);
  }
    ...
    if (msg.role === "user") {
      ...
      for (const block of msg.content) {
        if (block.type === "text") {
          ...
          try {
            input = JSON.parse(
              remapArgsCodexToClaude(block.name, block.arguments),
            );
          } catch {
      ...
      if (session.usage) {
        (assistantLine.message as Record<string, unknown>).usage =
          session.usage;
      }

      if (gitBranch) assistantLine.gitBranch = gitBranch;
      if (slug) assistantLine.slug = slug;
