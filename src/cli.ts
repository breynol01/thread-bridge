...
import {
  listCodexSessions,
  resolveCodexSession,
} from "./discovery/codexDiscovery.js";
import {
  listClaudeSessions,
  resolveClaudeSession,
} from "./discovery/claudeDiscovery.js";
import { opencodeProvider } from "./providers/opencode.js";
import { getUnifiedIndex } from "./unified/index.js";
import type { ProviderName } from "./providers/types.js";

function providerBadge(provider: ProviderName): string {
  switch (provider) {
 ...
 */
function resolveAnySession(
  idPrefix: string,
  sourceHint?: "codex" | "claude",
): { source: "codex" | "claude"; id: string; filePath: string } | undefined {
  if (sourceHint === "codex" || !sourceHint) {
    const codex = resolveCodexSession(idPrefix);
    if (codex) return { source: "codex", ...codex };
  }
  if (sourceHint === "claude" || !sourceHint) {
    const claude = resolveClaudeSession(idPrefix);
    if (claude) return { source: "claude", ...claude };
  }
  return undefined;
}
...
export function createCli(): Command {
  ...
  list
    ...
    .action((opts: { project?: string }) => {
      ...
      for (const s of sessions.slice(0, 25)) {
        ...
        const hasFile = s.filePath ? chalk.green("*") : chalk.red("!");
        console.log(
          `  ${hasFile} ${chalk.cyan(s.sessionId)}  ${display}  ${chalk.dim(date)}`,
        );
      }
      if (sessions.length > 25) {
        console.log(chalk.dim(`  ... and ${sessions.length - 25} more`));
  ...
  convert
    ...
    .action(
      (sessionId: string | undefined, opts: { dryRun?: boolean; cwd?: string; name?: string }) => {
        if (!sessionId) {
          const sessions = listCodexSessions();
          if (sessions.length === 0) {
            console.log(chalk.yellow("No Codex sessions found."));
        ...
        console.log(
          chalk.dim(
            `Parsed ${ir.messages.length} messages (model: ${ir.sourceModel ?? "unknown"})`,
          ),
        );
  ...
  program
    ...
    .action((sessionId: string, opts: { source?: string }) => {
      const sourceHint = opts.source as "codex" | "claude" | undefined;
