...
export function opencodeStorageDbPath() {
...
export function codexArchivedSessionsDir() {
...
export function codexHistoryPath() {
    return join(codexHome(), "history.jsonl");
}
export function claudeHistoryPath() {
 ...
 */
export function codexSessionFilePath(sessionId, date) {
    const yyyy = date.getUTCFullYear();
    const mm = String(date.getUTCMonth() + 1).padStart(2, "0");
    const dd = String(date.getUTCDate()).padStart(2, "0");
    const ts = date.toISOString().replace(/[:.]/g, "-").slice(0, 19);
    return join(codexSessionsDir(), String(yyyy), mm, dd, `rollout-${ts}-${sessionId}.jsonl`);
}
/**
 * Build the session file path for a Claude Code session.
 */
export function claudeSessionFilePath(projectPath, sessionId) {
 ...
 */
export function claudeSessionEnvDir(sessionId) {
    return join(claudeHome(), "session-env", sessionId);
}
const ANTHROPIC_ID_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
/**
 * Generate an ID matching Anthropic's format.
 * msg_01 + 24 alphanumeric chars, or req_01 + 24 alphanumeric chars.
 */
export function generateAnthropicId(prefix) {
...
export function normalizeProjectDir(dir) {
    ...
    if (!dir.startsWith(worktreePrefix))
        return dir;
